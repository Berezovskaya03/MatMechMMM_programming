//дана аудитория и n человек, которые хотят читать в ней лекции. Каждый человек указывает время лекции от a_i до b_i. Организовать расписание, при котором наибольшее количество занятий будет проведено
#include <iostream>
#include <cmath>
using namespace std;
struct segmentspart{
...
int main() {
    ...
    quickSort(seg, 0, n-1);//ñîðòèðîâêà ïî êîíöàì îòðåçêîâ â ïîðÿäêå íåóáûâàíèÿ
    ...
    int counter=0;
    for(int i = 1;i<n;i++){//æàäíûé àëãîðèòì ïî òåîðåìå ïî ïîèñêó ìèíèìàëüíîé áàçû ìàòðîèäà
